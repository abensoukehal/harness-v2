...
const compiled = Object.fromEntries(Object.entries(schemas).map(([name, s]) => [name, ajv.compile(s)]));
...
export function validate(data, schema) {
  ...
  if (!check(data)) return { ok: false, errors: format(check.errors) };
  const errors = schema === "config" ? checkConfig(data) : checkState(data);
...
function checkConfig(cfg) {
  ...
  for (const [name, s] of Object.entries(cfg.stacks)) {
    ...
    if (s.path !== `repos/${s.repo}` && !s.path.startsWith(`repos/${s.repo}/`)) {
    ...
    const strings = [...Object.values(s.commands), s.dev_url, s.health.http, s.seed].filter(Boolean);
  ...
  for (const key of ["client_tests", "test_runner"]) {
