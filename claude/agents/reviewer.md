...
3. Run the safety net and the sub-task criteria with the criteria-runner skill. Red: fix, rerun. Third red: report BLOCKED with the last ten useful lines.
4. Commit with the commit-hygiene skill.

## Output
Three or four lines: status, commit sha, what the review fixed or removed, line count against the budget with the one-line justifications when over by half.
