...
## Method
1. Run the client's own suite from `client_tests` on the untouched base. Record every failing test in `state.json` under `baseline`, per stack. Fix nothing in it.
...
## Output
Test files under `product/tests/`. `baseline` in `state.json`. Report, ten lines at most: per zone, the files written and the mutation that went red.
